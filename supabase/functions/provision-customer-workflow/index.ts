...
function findBestSubmitSourceNode(workflow: any) {
  ...
  const candidates = selection.candidates
    ...
    .join(", ");
  throw new Error(
    `Could not safely restore Nexus Submit Output${candidates ? ` from: ${candidates}` : ""}. Rename the exact buyer result node to NEXUS_FINAL_OUTPUT, sync it, and retry.`,
  );
}
...
function ensureSubmitOutputUsesRuntimePayload(workflow: any, callbackUrl: string) {
  ...
  const selectedSourceNode = findBestSubmitSourceNode({
    ...workflow,
    connections,
  });
  ensureMainConnection(
    connections,
  ...
  */
  submitNode.parameters = {
    ...
    specifyBody: "keypair",
    bodyParameters: buildBuyerOutputBodyParameters(),
    options: {},
  };
...
function validateCopiedWorkflow(workflow: any) {
  ...
  const submitOutputNode = nodes.find((node: any) => node?.name === "Nexus Submit Output");
  const hasSubmitOutput = Boolean(submitOutputNode);
  ...
  const submitOutputContract = JSON.stringify(submitOutputNode?.parameters || {});
  const missingSubmitOutputFields = [
  ...
  if (missingSubmitOutputFields.length) {
    throw new Error(
      `Copied workflow has an unsafe Nexus Submit Output contract. Missing: ${missingSubmitOutputFields.join(", ")}.`,
    );
  }
