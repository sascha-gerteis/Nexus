...
function normalizeWorkflow(product: any, rawWorkflow: any, supabaseUrl: string, n8nBaseUrl: string) {
  ...
  */
  ...
  const outputSelection = selectBuyerOutputNode({ nodes, connections });
  const attachToNodeName = cleanString(outputSelection.node?.name);

  if (!attachToNodeName) {
    const candidateSummary = outputSelection.candidates
      .slice(0, 6)
      .map((candidate: any) => `${candidate.name} (${candidate.score})`)
      .join(", ");
    throw new Error(
      `Could not safely detect the buyer-facing final output${candidateSummary ? ` from: ${candidateSummary}` : ""}. Rename the node that produces the exact buyer result to NEXUS_FINAL_OUTPUT. Nexus will not guess between ambiguous text, HTML, file, or notification branches.`,
    );
  }
