...
function normalizeCredentialMatchKey(value: unknown) {
  return cleanString(value)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "");
}

function credentialOwnershipText(field: any) {
  return [
    field?.name,
    field?.key,
...
function isKnownBuyerAccessCredential(field: any) {
  const text = credentialOwnershipText(field);
  return /\b(meta|facebook|instagram|whatsapp)\b/.test(text) &&
    /\b(access[\s_-]*token|token|credential|secret)\b/.test(text);
}

function isBuyerOwnedCredentialField(field: any, automation: any) {
  if (!field || typeof field !== "object" || Array.isArray(field)) return false;

  const owner = cleanString(field.owner || field.credential_owner || field.credentialOwner || field.scope).toLowerCase();
  const mode = cleanString(field.mode || field.credential_mode || field.credentialMode).toLowerCase();
  const source = cleanString(field.source || field.source_type || field.sourceType).toLowerCase();
  const visibility = cleanString(field.visibility || field.audience || field.exposed_to).toLowerCase();
  const ownershipText = credentialOwnershipText(field);

  if (
    field.developer_owned === true ||
    field.internal === true ||
    field.admin_only === true ||
    ownershipText.includes("developer-owned") ||
    ownershipText.includes("developer owned") ||
    ["developer", "dev", "admin", "nexus", "internal"].includes(owner) ||
    ["developer_owned", "private_per_customer"].includes(mode) ||
    ["developer", "admin", "nexus", "internal"].includes(source)
  ) {
    return false;
  }

  const fieldName = normalizeCredentialMatchKey(field.name || field.key || field.credential_key);
  const devSlots = normalizeJsonArray(automation?.developer_credential_requirements);
  const devSlotMatches = Boolean(fieldName) && devSlots.some((slot: any) => {
    const keys = [
      slot?.credential_key,
      slot?.key,
      slot?.provider,
      ...
      slot?.node_name,
    ].map(normalizeCredentialMatchKey).filter(Boolean);
    return keys.some((key) => key === fieldName || key.includes(fieldName) || fieldName.includes(key));
  });

  if (devSlotMatches && !isKnownBuyerAccessCredential(field)) return false;
  ...
  return (
    ...
    ["customer", "buyer"].includes(owner) ||
    mode === "customer_owned" ||
    ["customer", "buyer"].includes(source) ||
    ["customer", "buyer"].includes(visibility)
  );
}

function buyerCredentialSchema(credentialSchema: any[], automation: any) {
  return credentialSchema.filter((field: any) => isBuyerOwnedCredentialField(field, automation));
}
...
async function createAutomationRun(
  ...
  if (result.error || !result.data?.id) {
    throw new Error(
      "Could not create the tracked automation run: " +
        (result.error?.message || "The run row was not returned."),
    );
  }
...
async function enqueueRuntimeDispatch(
  adminClient: any,
  payload: {
    runId: string;
    customerAutomationId: string;
    setupSubmissionId: string;
    origin?: "setup_submit" | "scheduled" | "manual";
  },
) {
  ...
  const { data, error } = await adminClient
    ...
    .insert({
      ...
      customer_automation_id: payload.customerAutomationId,
      setup_submission_id: payload.setupSubmissionId,
      dispatch_origin: payload.origin || "setup_submit",
      status: "pending",
      attempt_count: 0,
  ...
  if (error || !data?.id) {
    throw new Error(
      `Could not create the durable runtime dispatch backlog: ${
        error?.message || "The queue row was not returned."
      }`,
    );
  }
...
async function acceptRuntimeDispatch(adminClient: any, runId: string) {
  const { error } = await adminClient
    .from("runtime_dispatch_queue")
    .update({
      status: "accepted",
  ...
  if (error) throw new Error(`Could not mark the runtime dispatch accepted: ${error.message}`);
}

async function deferRuntimeDispatch(
  adminClient: any,
  params: {
    runId: string;
    customerAutomation: any;
  ...
  const { error: queueError } = await adminClient
    ...
    .update({
      ...
      locked_at: null,
      worker_id: null,
      last_error_code: "RUNTIME_TEMPORARILY_UNAVAILABLE",
      last_error_message: params.message,
      last_error_details: { retrying: true, source: "submit-automation-setup" },
      updated_at: now,
    })
    .eq("run_id", params.runId)
    .in("status", ["pending", "processing"]);

  if (queueError) throw new Error(`Could not preserve the runtime retry backlog: ${queueError.message}`);

  await updateAutomationRunById(adminClient, params.runId, {
    status: "queued",
    finished_at: null,
    updated_at: now,
    error_message: null,
    response_payload: { status: "queued_for_retry", next_attempt_at: nextAttemptAt },
  });

  if (params.bundleRunItemId) {
    await updateBundleRunItem(adminClient, params.bundleRunItemId, {
      status: "queued",
      finished_at: null,
      error_message: null,
      updated_at: now,
  ...
  await safeUpdateCustomerAutomation(adminClient, params.customerAutomation.id, {
    status: "queued",
  ...
  await insertAutomationEvent(adminClient, {
    ...
    event_type: "runtime_dispatch_queued",
    title: "Automation safely queued",
    message: JSON.stringify({ run_id: params.runId, next_attempt_at: nextAttemptAt, reason: params.message }),
    created_by: "runtime",
  });
...
async function deadLetterRuntimeDispatch(adminClient: any, runId: string, message: string) {
  const { error } = await adminClient
    .from("runtime_dispatch_queue")
    .update({
      status: "dead_letter",
      locked_at: null,
      worker_id: null,
      last_error_code: "CUSTOMER_SETUP_INVALID",
  ...
  if (error) console.warn("runtime dispatch dead-letter update failed:", error.message);
}
  ...
  try {
    response = await fetch(params.webhookUrl, {
      ...
      headers: {
        ...
        "x-nexus-run-key": params.runKey || "",
        "Idempotency-Key": params.runKey || "",
      },
  ...
  try {
    ...
    */
    ...
    /*
      ...
      Buyer submissions are queued before any runtime request and only move to
      running after the runtime accepts the exact run. Admin setup-only
      submissions save values without firing the runtime yet.
    */
    ...
    if (shouldProvisionCustomerWorkflow) {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error || "");
        const provisioningMessage = `Customer workflow provisioning failed before trigger: ${message}`;
        const immediateClassification = classifyImmediateWebhookError(provisioningMessage);

        if (!immediateClassification.needs_customer_action) {
          const queuedAt = new Date().toISOString();
          const queuedRunKey = `setup:${customerAutomation.id}:${submission.id}`;
          const queuedBundleAttemptId = isUuid(bundleAttemptId) ? bundleAttemptId : "";
          let queuedBundleRunItemId = "";

          if (queuedBundleAttemptId) {
            await upsertBundleRunAttempt(adminClient, {
              id: queuedBundleAttemptId,
            ...
            const queuedBundleItem = await upsertBundleRunItem(adminClient, {
              bundle_run_attempt_id: queuedBundleAttemptId,
              order_id: bundleOrderId || customerAutomation.order_id || null,
              bundle_id: submittedBundleId || customerAutomation.bundle_id || null,
              buyer_id: customerAutomation.buyer_id || user.id || null,
              customer_automation_id: customerAutomation.id,
            ...
            queuedBundleRunItemId = cleanString(queuedBundleItem?.id);
          }

          if (orderIsBundle && (!isUuid(queuedBundleAttemptId) || !isUuid(queuedBundleRunItemId))) {
            return errorResponse(
              "Nexus could not establish exact bundle run tracking. No workflow was started.",
              500,
            );
          }

          const queuedRun = await createAutomationRun(adminClient, {
            customer_automation_id: customerAutomation.id,
            buyer_id: customerAutomation.buyer_id,
            automation_id: customerAutomation.automation_id,
            order_id: customerAutomation.order_id,
            ...
            bundle_run_item_id: queuedBundleRunItemId || null,
            request_payload: {
          ...
          const queuedDispatch = await enqueueRuntimeDispatch(adminClient, {
            runId: queuedRun.id,
            customerAutomationId: customerAutomation.id,
            setupSubmissionId: submission.id,
            origin: "setup_submit",
          });

          if (queuedBundleRunItemId) {
            await updateBundleRunItem(adminClient, queuedBundleRunItemId, {
              automation_run_id: queuedRun.id,
              status: "queued",
              updated_at: queuedAt,
            });
          }

          const nextAttemptAt = await deferRuntimeDispatch(adminClient, {
            runId: queuedRun.id,
            customerAutomation,
            bundleRunItemId: queuedBundleRunItemId,
            message: provisioningMessage,
          });

          if (queuedBundleAttemptId) {
            await refreshBundleRunAttemptRollup(adminClient, queuedBundleAttemptId);
          }
          ...
          return jsonResponse({
            ...
            submission_id: submission.id,
            run_id: queuedRun.id,
            run_key: queuedRun.run_key || queuedRunKey,
            dispatch_queue_id: queuedDispatch.id,
            next_attempt_at: nextAttemptAt,
            setup_keys: Object.keys(setupAnswers),
            credential_keys_available: Object.keys(savedSecrets),
            message:
    ...
    try {
      ...
      if (triggerResult) {
        ...
        await deadLetterRuntimeDispatch(adminClient, activeRun.id, message);
