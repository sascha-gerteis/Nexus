...
function isCredentialLikeFieldName(value: unknown) {
  ...
  const providerSpecificSecret = Boolean(
    /(?:^|_)(?:token|secret|password|key)$/.test(name) &&
    providerPreset(name)
  );
...
export function credentialCarriersForWorkflow(nodes: any[]) {
...
export function httpCredentialReferenceHint(node: any, credentialCarriers: Map<string, any> | null = null) {
