  ...
  return {
    ...
    ...(source === "credential"
      ? {
        ...
        developer_owned: true,
        owner: "developer",
        visibility: "internal",
        schema_source: "python_inference",
      }
      : {}),
  };
}

function markPythonCredentialFieldDeveloperOwned(field: any) {
  ...
  return {
    ...
    owner: "developer",
    visibility: "internal",
    schema_source: cleanString(field.schema_source) || "python_credential_schema",
  };
}
...
function buildProductPayload(body: Record<string, unknown>, developerId: string, status: "draft" | "pending_review", slug: string) {
  ...
  const credentialSchema = isPythonProduct
    ? mergeSchemaFields(cleanSchema(body.credential_schema), pythonInference.credentials)
      .map(markPythonCredentialFieldDeveloperOwned)
