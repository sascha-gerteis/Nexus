const NexusDB = (() => {
  ...
  async function buyerSignUp(email, password, metadata = {}, nextUrl = "", reason = "") {
    clearAuthCaches();

    const confirmationRedirect = new URL(buyerAuthRedirectUrl(nextUrl, reason));
    confirmationRedirect.searchParams.set("confirmed", "1");
...
async function listBuyerAutomationRuns(userId) {
  ...
  const withBundleColumns = await supabase
    .from("automation_runs")
    .select(`
      id,
      customer_automation_id,
  ...
  if (!withBundleColumns.error || !isSchemaError(withBundleColumns.error)) {
    return withBundleColumns;
  }
...
async function listBuyerBundleRunAttempts(userId) {
  let buyerId = userId || "";
...
async function checkN8nExecution(payload = {}) {
  return callNexusFunction("check-n8n-execution", payload);
}
...
async function getComplimentaryGrantOptions() {
  return callNexusFunction("nexus-install-request", {
    action: "admin_grant_options"
  });
}

async function grantComplimentaryProduct(payload) {
  return callNexusFunction("nexus-install-request", {
    action: "admin_grant_product",
    ...payload
  });
}
...
getComplimentaryGrantOptions,
grantComplimentaryProduct,
