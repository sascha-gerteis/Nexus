...
let generatedAccessibleFieldId = 0;
let accessibilityObserver = null;

function accessibleControlNearLabel(label) {
  const selector = "input:not([type='hidden']), select, textarea";
  if (label.querySelector(selector)) return null;

  let sibling = label.nextElementSibling;
  let inspected = 0;

  while (sibling && inspected < 4) {
    if (sibling.matches?.("label")) break;
...
function associateFormLabels(root = document) {
  ...
  if (scope.matches?.("label:not([for])")) labels.push(scope);
  labels.push(...scope.querySelectorAll("label:not([for])"));

  labels.forEach((label) => {
    const control = accessibleControlNearLabel(label);
    if (!control) return;

    if (!control.id) {
      generatedAccessibleFieldId += 1;
      control.id = `nexus-field-${generatedAccessibleFieldId}`;
    }
...
function startAccessibilityObserver(root = document) {
  associateFormLabels(root);

  if (accessibilityObserver || typeof MutationObserver === "undefined" || !document.body) return;

  accessibilityObserver = new MutationObserver((records) => {
    records.forEach((record) => {
      record.addedNodes.forEach((node) => {
        if (node?.nodeType === Node.ELEMENT_NODE) associateFormLabels(node);
      });
    });
  });
...
async function bootNexusUI() {
  ...
  NexusUI.startAccessibilityObserver?.();
