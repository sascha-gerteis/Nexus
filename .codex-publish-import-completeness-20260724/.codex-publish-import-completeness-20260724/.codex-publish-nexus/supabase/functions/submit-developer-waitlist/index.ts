...
async function queueWaitlistConfirmation(
  adminClient: any,
  payload: {
    name: string;
    email: string;
    automationCategories: string[];
    buildStack: string[];
    buildStackOther: string;
  },
) {
  await safeEnqueueEmail(
    adminClient,
    "developer_waitlist_confirmation",
    { email: payload.email, name: payload.name },
    {
      name: payload.name,
      automation_categories: payload.automationCategories,
      build_stack: payload.buildStack,
      build_stack_other: payload.buildStackOther,
    },
    {
      dedupeKey: `developer_waitlist_confirmation:${payload.email}`,
    },
  );
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!error) {
      ...
      await queueWaitlistConfirmation(adminClient, {
        name,
        email,
