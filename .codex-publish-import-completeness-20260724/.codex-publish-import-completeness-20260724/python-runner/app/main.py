...
def looks_like_html(value):
    if not isinstance(value, str):
        return False
    return bool(re.search(r"<!doctype\\s+html|<html[\\s>]|<(?:body|main|section|article|table|div|h1|h2|p)[\\s>]", value, re.I))
    ...
    for key in [
        ...
        if nested not in (None, "", [], {{}}):
            if isinstance(nested, dict):
                merged = {{**nested}}
                for metadata_key in ["status", "output_type", "title", "summary"]:
                    if metadata_key in value and metadata_key not in merged:
                        merged[metadata_key] = value[metadata_key]
                return merged
            return nested
    return value

def output_score(raw):
    value = unwrap_output(raw)
    ...
    points = 0
    html_value = (
        value.get("content_html") or
        value.get("contentHtml") or
        value.get("html") or
        value.get("report_html") or
        value.get("reportHtml")
    )
    if html_value:
        points += 900 if looks_like_html(str(html_value)) else 650
    if value.get("file_url") or value.get("fileUrl") or value.get("storage_path") or value.get("storagePath"):
        points += 600
    if value.get("content_json") or value.get("contentJson") or value.get("output_type") or value.get("outputType"):
        points += 300
    if value.get("title") and value.get("summary"):
        points += 180
    if value.get("content_text") or value.get("contentText") or value.get("text") or value.get("markdown"):
        points += 100
    return points

def select_rich_output(value):
    if isinstance(value, list) and value:
        return max(value, key=output_score)
    if isinstance(value, dict):
        candidates = [value]
        for key in ["outputs", "results", "reports", "items"]:
            nested = value.get(key)
            if isinstance(nested, list):
                candidates.extend(nested)
        return max(candidates, key=output_score)
    return value

def normalize_result(value):
    value = unwrap_output(select_rich_output({{}} if value is None else value))
    ...
    if isinstance(value, str):
        ...
        return {{
            ...
            "summary": "" if is_html else value[:500],
            "content_text": "" if is_html else value,
            "content_html": value if is_html else "",
            "content_json": {{"html": value}} if is_html else {{"text": value}},
        }}
    ...
    content_html = (
        ...
        value.get("html") or
        value.get("report_html") or
        value.get("reportHtml") or
        ""
    )
    content_text = (
        value.get("content_text") or
        value.get("contentText") or
        value.get("text") or
        value.get("markdown") or
        ""
    )
    ...
    return {{
        ...
        "output_type": value.get("output_type") or value.get("outputType") or value.get("type") or "report",
        "title": value.get("title") or value.get("report_title") or value.get("reportTitle") or value.get("name") or "Automation output",
        "summary": value.get("summary") or value.get("description") or "",
        "content_html": content_html,
        "content_text": content_text,
        "file_url": value.get("file_url") or value.get("fileUrl") or "",
        "storage_path": value.get("storage_path") or value.get("storagePath") or "",
