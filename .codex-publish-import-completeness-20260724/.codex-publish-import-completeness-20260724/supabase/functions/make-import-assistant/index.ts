...
function buildN8nWorkflow(product: any, groups: any[], mappings: any[], platform = "make") {
  ...
  const outputNode = makeCodeNode(
    ...
    groups.length + 2,
    [
      "const candidates = items.map((item) => item?.json ?? item).filter((value) => value !== undefined && value !== null);",
      "const looksLikeHtml = (value) => typeof value === 'string' && /<!doctype\\s+html|<html[\\s>]|<(?:body|main|section|article|table|div|h1|h2|p)[\\s>]/i.test(value);",
      "const unwrap = (value) => {",
      "  if (!value || typeof value !== 'object' || Array.isArray(value)) return value;",
      "  return value.NEXUS_FINAL_OUTPUT || value.nexus_final_output || value.final_output || value.finalOutput || value.output || value.result || value.report || value.payload || value.data || value;",
      "};",
      "const score = (raw) => {",
      "  const value = unwrap(raw);",
      "  if (looksLikeHtml(value)) return 900;",
      "  if (!value || typeof value !== 'object' || Array.isArray(value)) return typeof value === 'string' ? 100 : 0;",
      "  let points = 0;",
      "  if (value.content_html || value.contentHtml || value.html || value.report_html || value.reportHtml) points += 900;",
      "  if (value.file_url || value.fileUrl || value.storage_path || value.storagePath) points += 600;",
      "  if (value.content_json || value.output_type || value.outputType) points += 300;",
      "  if (value.title && value.summary) points += 180;",
      "  if (value.content_text || value.contentText || value.text || value.markdown) points += 100;",
      "  return points;",
      "};",
      "const raw = candidates.sort((left, right) => score(right) - score(left))[0] || {};",
      "const first = unwrap(raw);",
      "const objectOutput = first && typeof first === 'object' && !Array.isArray(first) ? first : {};",
      "const textOutput = typeof first === 'string' ? first : '';",
      "const contentHtml = objectOutput.content_html || objectOutput.contentHtml || objectOutput.html || objectOutput.report_html || objectOutput.reportHtml || (looksLikeHtml(textOutput) ? textOutput : '');",
      "const contentText = objectOutput.content_text || objectOutput.contentText || objectOutput.text || objectOutput.markdown || (!contentHtml ? textOutput : '');",
      "return [{ json: {",
      "  status: objectOutput.status || 'success',",
      "  output_type: objectOutput.output_type || objectOutput.outputType || 'report',",
      "  title: objectOutput.title || objectOutput.report_title || objectOutput.name || 'Automation output',",
      `  summary: objectOutput.summary || objectOutput.description || 'Converted ${label} workflow completed successfully.',`,
      ...
      "  content_json: objectOutput.content_json || objectOutput.contentJson || objectOutput || { value: first },",
      "  raw_result: raw",
      "} }];",
    ].join("\n"),
  );
