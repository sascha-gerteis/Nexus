...
for (const [source, marker, label] of [
  ...
  [buyerPage, 'const eventHeader = live ? ""', "live cURL omits stale IDs so Nexus generates a fresh identity"],
  [buyerPage, "Every request will receive a new Nexus event ID", "buyer-visible live duplicate guidance"],
  [buyerPage, "latestPayload = webhookState.config?.inbound_last_payload_preview", "live example reuses the payload that passed mapping"],
  [buyerPage, "Optional retry protection:", "event ID correctly documented as optional for live requests"],
