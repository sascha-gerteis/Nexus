...
for (const [source, marker, label] of [
  ...
  [buyerDashboard, "const webhookRuns = webhookProduct", "webhook latest output resolves against its own run history"],
  [buyerDashboard, "const currentAutomationOutputs = automationOutputs;", "webhook automation card preserves output history"],
  [buyerDashboard, "const currentOutputs = outputs;", "webhook automation detail preserves output history"],
  [buyerDashboard, "matchingRuns.find(run => outputMatchesRun(output, run))", "webhook output library validates each historical run"],
