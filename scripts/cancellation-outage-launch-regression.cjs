...
[
  ...
  [reviewCancellation, '.eq("order_id", orderId)', "bundle-wide cancellation"],
  [reviewCancellation, 'payment_status: "refunded"', "refunded order state"],
  [reviewCancellation, 'next_run_at: null', "future runs stopped"],
  [reviewCancellation, "markCustomerAutomationsCancelled", "schema-compatible cancellation state update"],
  [reviewCancellation, ".update(coreUpdates)", "core cancellation update fallback"],
  ...
  [listCancellation, "orders(*)", "schema-compatible cancellation order read"],
