...
if (typeof NexusUI.refreshUsdToThbRate === "function") {
  ...
  function closeMarketplaceModal() {
    document.getElementById("productModal")?.classList.remove("bundle-modal-view");
    NexusUI.closeModal();
  ...
  async function openBundle(slug) {
    ...
    document.getElementById("productModal")?.classList.add("bundle-modal-view");
    setMarketplaceShareUrl("bundle", bundle.slug || slug);

    const products = Array.isArray(bundle.bundle_products) ? bundle.bundle_products : [];
    const bundlePricingType = String(bundle.pricing_type || "monthly").toLowerCase() === "one_time"
      ? "one_time"
      : "monthly";
    const bundleBillingLabel = bundlePricingType === "one_time" ? "One-time payment" : "Monthly subscription";
    const bundleBillingSentence = bundlePricingType === "one_time"
      ? "The selected workflows are charged once."
      : "The selected workflows are included in one recurring monthly subscription.";
    ...
    const side = `
      ...
      <div class="price-box">
        ...
        <small>${NexusUI.escapeHtml(bundleBillingLabel)}. ${Number(bundle.discount_percent || 0)}% bundle discount applied to active included products.</small>
      </div>
      ...
      <div class="product-maintenance-panel">
        ...
        <div>
          <span>Billing</span>
          <strong>${NexusUI.escapeHtml(bundleBillingLabel)}</strong>
        </div>
    ...
    if (totalNote) {
      if (bundlePricing) {
        const discountLabel = bundlePricing.discountPercent > 0
          ...
          : "";
        const cadenceLabel = pricingType === "one_time" ? "One-time payment." : "Billed monthly.";
        totalNote.textContent = `${bundlePricing.selected.length} of ${bundlePricing.products.length} workflows selected${discountLabel}. ${cadenceLabel} The price and setup include only these workflows.`;
