...
function isNativeN8nCredentialSlot(slot: any) {
  ...
  return Boolean(type && nodeType && !isGenericHttpCredentialType(type) && !isHttpRequestNodeType(nodeType));
}

function requiresNativeEditorCredentialSetup(slot: any) {
  const type = lower(slot?.n8n_credential_type || slot?.credential_key);
  if (!isNativeN8nCredentialSlot(slot)) return false;
...
function manualNativeBindingFromSlot(slot: any) {
  const credentialId = cleanString(slot?.current_id);
  const credentialName = cleanString(slot?.current_name);
  if (!isNativeN8nCredentialSlot(slot) || (!credentialId && !credentialName)) return null;
  ...
  return {
    ...
    n8n_credential_id: credentialId || null,
    n8n_credential_name: credentialName || "n8n account credential",
    developer_credential_id: null,
    manual_n8n_credential: true,
  };
}

function nativeCredentialSetupMessage(slot: any) {
  const provider = cleanString(slot?.provider_label || slot?.provider || slot?.n8n_credential_type || "provider");
  const node = cleanString(slot?.node_name || "the workflow node");
  const nodeType = cleanString(slot?.node_type);
  const type = lower(slot?.n8n_credential_type || slot?.credential_key);
  const hints = Array.isArray(slot?.setup_hints)
    ? slot.setup_hints.map(cleanString).filter(Boolean).join(" ")
    : "";

  if (type === "googleapi") {
    return `Add a Google Service Account credential in Nexus for "${node}". Paste the service account JSON or email/private key, share the target Google Sheet/Drive file/Doc with the service account email, then press Apply credentials & run check.`;
  }

  return `${provider} uses n8n's native account setup on "${node}". ${hints ? `${hints} ` : ""}Click Edit workflow, open "${node}"${nodeType ? ` (${nodeType})` : ""}, use Set up credential / select ${provider} account, save the workflow, then click Sync changes and Run check.`;
}
...
function bindingIsAppliedToLiveSlot(binding: any, slot: any) {
  if (!bindingMatchesSlot(binding, slot)) return false;
  if (binding?.uses_nexus_proxy) return true;

  const bindingId = cleanString(binding?.n8n_credential_id);
  const bindingName = cleanString(binding?.n8n_credential_name);
  const slotId = cleanString(slot?.current_id);
  const slotName = cleanString(slot?.current_name);

  if (bindingId && slotId && bindingId === slotId) return true;
  if (bindingName && slotName && bindingName === slotName) return true;
  return false;
}
...
function providerMatchesCredential(credential: any, slot: any) {
  ...
  const googleAccountProviders = new Set([
    "google_sheets",
    "google_drive",
    "google_calendar",
    "google_docs",
  ...
  const googleAccountCredentialTypes = new Set([
  ...
  const credentialIsGoogleAccount = googleAccountProviders.has(credentialTypePreset?.provider || credentialProvider) ||
    googleAccountProviders.has(credentialProvider) ||
    googleAccountCredentialTypes.has(lower(credential?.n8n_credential_type));
  const slotIsGoogleAccount = googleAccountProviders.has(slotTypePreset?.provider || slotProvider) ||
    googleAccountProviders.has(slotProvider) ||
    googleAccountCredentialTypes.has(lower(slot?.n8n_credential_type || slot?.credential_key));
  const slotCredentialType = lower(slot?.n8n_credential_type || slot?.credential_key);
  const credentialType = lower(credential?.n8n_credential_type);

  if (slotProvider === "google_service_account" || slotCredentialType === "googleapi") {
    return credentialProvider === "google_service_account" || credentialType === "googleapi";
  }
  ...
  if (credentialProvider === slotProvider) return true;
  if (credentialIsGoogleAccount && slotIsGoogleAccount) return true;
...
async function saveCredential(adminClient: any, operator: any, body: any) {
  ...
  const canUseSuppliedN8nCredential = Boolean(suppliedN8nCredentialId);
  ...
  const patch = {
    ...
    n8n_credential_id: canKeepExistingN8nCredential || canUseSuppliedN8nCredential
      ? suppliedN8nCredentialId || existing?.n8n_credential_id || null
      : null,
    n8n_credential_name: canKeepExistingN8nCredential || canUseSuppliedN8nCredential
      ? suppliedN8nCredentialName || existing?.n8n_credential_name || n8nCredentialName
      : label,
    status: "active",
...
async function scanAutomation(adminClient: any, operator: any, body: any) {
  ...
  const sourceWorkflow = liveWorkflow
  ...
  const bindings = Array.isArray(product.n8n_credential_bindings)
    ? product.n8n_credential_bindings.filter((binding: any) => (
      (binding?.developer_credential_id || binding?.manual_n8n_credential) &&
      slots.some((slot: any) => liveWorkflow
        ? bindingIsAppliedToLiveSlot(binding, slot)
        : bindingMatchesSlot(binding, slot))
    ))
    : [];
  ...
  for (const slot of slots) {
    ...
    const manualNativeBinding = liveWorkflow ? manualNativeBindingFromSlot(slot) : null;
  ...
  const boundSlotKeys = new Set(
    bindings
      .filter((binding: any) => binding?.developer_credential_id || binding?.manual_n8n_credential)
  ...
  const errors = missingSlots.map((slot: any) => ({
    ...
    message: isNativeN8nCredentialSlot(slot)
      ? requiresNativeEditorCredentialSetup(slot)
        ? nativeCredentialSetupMessage(slot)
