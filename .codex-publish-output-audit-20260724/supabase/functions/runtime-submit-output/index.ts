...
async function findCallbackRunContext(adminClient: any, body: any, customerAutomationId: string) {
  ...
  const activeRuns = (data || []).filter((run: any) => callbackRunStatusIsActive(run?.status));
  if (activeRuns.length === 1) return activeRuns[0];

  if (activeRuns.length > 1) {
    console.warn("Runtime callback rejected because more than one active run could own it.");
  }

  // Never attach an identity-free callback to an old or terminal run.
  return null;
}
