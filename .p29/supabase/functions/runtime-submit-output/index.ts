...
function callbackRunStatusIsSuccessful(value: unknown) {
  const status = cleanString(value).toLowerCase();
  return ["success", "succeeded", "completed", "complete"].includes(status);
}

function selectLegacyStandaloneCallbackRun(
  rows: any[],
  expectedOrderId: string,
) {
  const orderId = cleanString(expectedOrderId);
  if (!orderId) return null;

  const candidates = (Array.isArray(rows) ? rows : []).filter((run: any) =>
    cleanString(run?.order_id) === orderId &&
    !cleanString(run?.bundle_run_attempt_id) &&
    !cleanString(run?.bundle_run_item_id)
  );

  /*
    Some hosted runners mark the Nexus run successful immediately before the
    legacy output node calls back. Accept only the newest row from this exact
    standalone purchase when it is active or successful; failed and cancelled
    rows must not fall back to an older retry.
  */
  const newestRun = candidates[0] || null;
  return callbackRunStatusIsActive(newestRun?.status) ||
      callbackRunStatusIsSuccessful(newestRun?.status)
    ? newestRun
    : null;
}
...
async function findCallbackRunContext(
  adminClient: any,
  body: any,
  customerAutomationId: string,
  expectedOrderId: string,
  ...
  return selectLegacyStandaloneCallbackRun(data || [], expectedOrderId);
}
...
async function updateExistingRunFromCallback(
  ...
  const runId = cleanString(resolvedRunId);
  if (!runId || !cleanString(customerAutomationId)) return false;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const callbackRunContext = await findCallbackRunContext(
      adminClient,
      body,
      customerAutomation.id,
      cleanString(customerAutomation.order_id),
    );
    ...
    if (!callbackRunContext?.id || !callbackAutomationRunId || !callbackOrderId) {
      return errorResponse(
        "Output callback does not match one exact Nexus order and automation run. The output was rejected to prevent stale or cross-purchase attribution.",
        409,
      );
    }

    if (
      cleanString(callbackRunContext.customer_automation_id) !== cleanString(customerAutomation.id) ||
      cleanString(callbackRunContext.order_id) !== callbackOrderId
    ) {
      return errorResponse(
        "Output callback identity does not match its Nexus purchase. The output was rejected.",
        409,
      );
    }
    ...
    if (isErrorStatus) {
      ...
      if (!updatedExistingRun) {
        console.error(JSON.stringify({
          event: "runtime_callback_run_update_failed",
          customer_automation_id: customerAutomation.id,
          run_id: callbackAutomationRunId,
          callback_status: status,
        }));
        return errorResponse(
          "Nexus could not update the exact automation run. The callback was not finalized and may be retried safely.",
          500,
        );
      }
      ...
      if (responseMode === "dashboard_output") {
        ...
        if (!updatedExistingRun) {
          console.error(JSON.stringify({
            ...
            customer_automation_id: customerAutomation.id,
            run_id: callbackAutomationRunId,
            callback_status: "empty_output",
          }));
          return errorResponse(
            "Nexus could not update the exact automation run. The rejected callback may be retried safely.",
            500,
          );
        }
    ...
    if (!updatedExistingRun) {
      console.error(JSON.stringify({
        ...
        customer_automation_id: customerAutomation.id,
        run_id: callbackAutomationRunId,
        callback_status: shouldWaitForN8nFinalStatus ? "output_received" : "success",
        output_id: output.id,
      }));
      return errorResponse(
        "Nexus saved the idempotent output but could not finalize the exact automation run. Retrying this callback is safe.",
        500,
      );
    }
