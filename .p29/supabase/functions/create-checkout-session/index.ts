...
function bundleCheckoutMode(bundle: any, products: any[]): "payment" | "subscription" {
  const pricingType = cleanString(bundle?.pricing_type).toLowerCase();
  if (pricingType === "monthly") return "subscription";
  if (pricingType === "one_time") return "payment";

  // Legacy bundles created before bundle-level billing inherit their included products.
...
async function loadBundleCheckout(
  ...
  const mode = bundleCheckoutMode(bundle, products);
...
async function createBundleCheckoutSession(options: {
  ...
  const bundleSnapshot = {
    ...
    bundle_icon: checkout.bundle.icon || "PK",
    bundle_color: checkout.bundle.color || "cyan",
    ...
    pricing_type: checkout.bundle.pricing_type || (checkout.mode === "subscription" ? "monthly" : "one_time"),
  ...
  const metadata = {
    ...
    price_source: checkout.priceSource,
    bundle_pricing_type: checkout.bundle.pricing_type || (checkout.mode === "subscription" ? "monthly" : "one_time"),
    payment_environment: paymentEnvironment,
    source: "nexus",
  };
