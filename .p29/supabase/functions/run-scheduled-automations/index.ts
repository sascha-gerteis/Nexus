...
async function enqueueScheduledDispatch(
  ...
  if (error || !data?.id) {
    throw new Error(`Could not create runtime dispatch backlog: ${error?.message || "Queue row missing."}`);
  }

  return data;
}

async function acceptScheduledDispatch(adminClient: any, runId: string) {
  const { error } = await adminClient
    .from("runtime_dispatch_queue")
    .update({
      status: "accepted",
  ...
  if (error) throw new Error(`Could not mark runtime dispatch accepted: ${error.message}`);
}

async function deferScheduledDispatch(
  adminClient: any,
  run: any,
  customerAutomation: any,
  message: string,
  ...
  const { error: queueError } = await adminClient
    ...
    .update({
      ...
      last_error_details: { retrying: true, source: "run-scheduled-automations" },
      updated_at: now,
    })
    .eq("run_id", run.id)
    .in("status", ["pending", "processing"]);

  if (queueError) throw new Error(`Could not preserve scheduled runtime backlog: ${queueError.message}`);
  ...
  await insertAutomationEvent(adminClient, {
    ...
    message: JSON.stringify({ run_id: run.id, run_key: run.run_key, next_attempt_at: nextAttemptAt }),
    created_by: "runtime",
  });
...
async function deadLetterScheduledDispatch(adminClient: any, runId: string, message: string) {
  await adminClient
    .from("runtime_dispatch_queue")
    .update({
      status: "dead_letter",
...
async function runCandidate(adminClient: any, row: any, options: {
  ...
  const dispatchQueue = await enqueueScheduledDispatch(
    adminClient,
    run,
    customerAutomation,
    isManualRun ? "manual" : "scheduled",
    asObject(options.eventPayload),
  );
  ...
  try {
    ...
    await acceptScheduledDispatch(adminClient, run.id);
    ...
    if (!classification.needsCustomerAction) {
      const nextAttemptAt = await deferScheduledDispatch(
        adminClient,
        run,
        customerAutomation,
        message,
      );
      let queuedNextRunAt: string | null = null;

      if (options.advanceSchedule) {
        queuedNextRunAt = nextScheduledDate(frequency, scheduledFor, new Date());
        await updateCustomerAutomation(adminClient, customerAutomation.id, {
          schedule_status: "active",
          schedule_anchor_at: customerAutomation.schedule_anchor_at || scheduledFor,
          next_run_at: queuedNextRunAt,
          updated_at: now,
        });
      }
    ...
    await deadLetterScheduledDispatch(adminClient, run.id, message);
