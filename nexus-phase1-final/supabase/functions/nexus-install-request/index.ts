...
async function listAdminOrders(adminClient: any) {
  ...
  const { data: automations, error: automationError } = await adminClient
    .from("automations")
    .select("id, title, slug, icon, color, status")
...
async function updateAdminInstallRequest(adminClient: any, userId: string, body: any) {
  ...
  */
  ...
  const { error: customerAutomationUpdateError } = await adminClient
    .from("customer_automations")
    .update(customerAutomationUpdates)
    .eq("id", customerAutomation.id);

  if (customerAutomationUpdateError) {
    return errorResponse(customerAutomationUpdateError.message, 500);
  }
